...
언어 자체가 싱글 스레드라는 말은 애매 모호하긴 한데 틀린 명제에 더 가까운 것 같다. 싱글 스레드는 언어가 구동되는 런타임 환경에 대한 설명이고, 언어 자체가 싱글 스레드인지 여부를 논하는 것은 좀 어렵다. 다만 언어가 싱글 스레드 환경을 염두해두고 설계되었다고 할 수 있다. 다만 스크립트 언어여도 멀티스레딩이 구현 가능한 환경이 제공되는 케이스도 있으며 (파이썬, 루비 등) 자바스크립트도 Node.js 에서 멀티스레딩을 지원하긴 한다. 또한 대부분의 언어가 설계되었을 당시에는 컴퓨터 하드디스크 사양이 지금처럼 발전되어 있지 않아서 크게 멀티스레딩 환경을 고려하지 않았을 수도 있다는 의견도 있었다. 멀티스레딩 개념 자체는 컴퓨터가 발명된 초창기부터 있어온 개념이기 때문에 반론하는 의견도 있었으므로 참고만 하면 될 것 같다.
