...
description: 리액트를 처음 배울 때는 Hook과 함수형 컴포넌트가 나온 지 얼마 되지 않을 때였다. 클래스로 컴포넌트 안에서 리액트 라이프사이클을 활용하여 컴포넌트 내의 동작을 제어하여 구현했다. 그러나 회사에서 실무를 하기 시작했을 때 개발팀의 의사 결정에 따라 함수형 컴포넌트 위주로 개발하게 되었고 이내 이 방식에 익숙해져버렸다.
